...
class SeqReplayBuffer:
    ...
    def clear(self):
        ...
        self._observations = np.zeros(
            (self._max_replay_buffer_size, self._observation_dim), dtype=np.float32
        )
        self._next_observations = np.zeros(
            (self._max_replay_buffer_size, self._observation_dim), dtype=np.float32
        )

        self._actions = np.zeros((self._max_replay_buffer_size, self._action_dim), dtype=np.float32)
